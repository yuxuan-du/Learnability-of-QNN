...
parser.add_argument('--noise', type=float, default=0.33, help='depolarization rate')
...
parser.add_argument('--num_mea', type=int, default=20, help='number of measurement for each iteration')
...
for epoch_ in range(args.epo_number_):
    ...
    for i in range(args.batch):
        ...
        mea_res = classfier.compute_predict_vector(fea_state, para, args.depolar , args.num_mea)
        ...
        for j in range(num_alpha):
            para_pos  = para.copy()
            para_pos[j] += np.pi / 2
            loss_pos[j] += classfier.compute_predict_vector(fea_state, para_pos, args.depolar , args.num_mea)
            para_neg  = para.copy()
            para_neg[j] -= np.pi / 2
            loss_neg[j] += classfier.compute_predict_vector(fea_state, para_neg, args.depolar , args.num_mea)
